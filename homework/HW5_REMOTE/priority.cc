...
*/
/*
// function to compare 2 processes by priority, breaking ties with arrival time, then burst
...
bool compare_burst_time2(vector<int> a, vector<int> b) {
...
void priority(vector<vector<int>> data) {
    ...
    while (!processes.empty() || !ready_queue.empty() || hasActiveProcess) {
        ...
        while (!processes.empty() && processes[0][1] == t) {
            ...
            sort(ready_queue.begin(), ready_queue.end(), compare_burst_time2);
        }

        if (!ready_queue.empty()) {
            // if there is an active process and the shortest ready process is shorter than the active process
            if (hasActiveProcess && ready_queue[0][2] < active_process[2]) {

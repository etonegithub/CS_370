...
def process_conn(conn):
    ...
    if "password" in msg:
        input_password_hash = hashlib.sha256(msg.get("password").encode('utf-8')).hexdigest()
        #print(input_password_hash == hashed_password)
    ...
    elif op == 'down' and input_password_hash == hashed_password:
        print(input_password_hash == hashed_password)
        filename = msg.get("filename")
        down(conn, filename)   
    ...
    elif op == 'ls' and input_password_hash == hashed_password:
        print(input_password_hash == hashed_password)
        list(conn)
    else:
...
def up(conn, filename):   
    ...
    conn.shutdown(socket.SHUT_WR) # close before sending file to api tester cuz that function takes a WHILE and there's no reason for the client to be sitting there connected the whole time
    conn.close()
    print("Disconnected")
...
# set local ip for pi
pi_ip = input("What is the local ip of the server? ") # CHANGE THIS BASED ON PI NETWORK IP (use ifconfig in py terminal to find pi ip). localhost ip is 127.0.0.1

# set port for pi
pi_port = int(input("What port are you using in the server? ")) # the port the pi is using on the network. should generally be fine with 8000, but if client connection is refused try a different port

# ask user to set password for server and hash it using SHA256
#old_password = "password" # obviously this isn't secure at all but just for a demo
password = input("What will the password be? ").encode('utf-8')
...
print("Server started on ip", pi_ip, "and port", pi_port, "...")
